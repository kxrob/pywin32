...
class RTDocument(RTParams, DocBase):
    ...
    def StartRun(self):
        ...
        thread.start_new(self.ThreadRun, ())
    ...
    def ThreadRun(self):
        ...
        try:
            ...
            if not enc:
                import _winreg
                k = _winreg.OpenKey(_winreg.HKEY_LOCAL_MACHINE, r'SYSTEM\CurrentControlSet\Control\Nls\CodePage')
                enc = 'cp' + _winreg.QueryValueEx(k, 'OEMCP')[0]
                k.Close()
                try: u''.encode(enc)
                except LookupError:
